...
const getTotalBalance = users => users.reduce((sum, user) => sum += user.balance, 0);
...
const getUniqueSkills = users =>
  users
    .reduce((skillAccum, user) => {
      skillAccum.push(...user.skills);
      return skillAccum;
    }, [])
    .reduce((uniqueSkill, currentSkill) => {
      if (!uniqueSkill.includes(currentSkill)) uniqueSkill.push(currentSkill);
      return uniqueSkill;
    }
      , [])
    .sort()
